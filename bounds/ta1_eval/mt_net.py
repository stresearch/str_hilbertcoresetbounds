...
class MT_CNN(nn.Module):
    ...
    def __init__(self, num_classes=10, isL2 = False, dropRatio = 0.0, input_channel=3):
        ...
        self.conv1a = weight_norm(nn.Conv2d(input_channel, 128, 3, padding=1))
        ...
        self.overclass_fc = weight_norm(nn.Linear(128, 70))
        self.fc2 =  weight_norm(nn.Linear(128, num_classes))
        self.softmax = nn.Softmax(dim=1)

    def forward(self, x, stage='B', debug=False):
        ...
        if stage == 'B':
            output = self.fc1(x)
            output_2 = self.fc2(x)
            output_after_softmax = self.softmax(output)
        else:
            output = self.overclass_fc(x)
            output_after_softmax = self.softmax(output)
        # return self.fc1(x), self.fc2(x), x
        return x, output, output_2, output_after_softmax
...
class ResNet(nn.Module):
    ...
    def forward(self, x, stage='B'):
        ...
        output = self.fc1(x)
        output_2 = self.fc2(x)
        return x, output, output_2, self.softmax(output)
